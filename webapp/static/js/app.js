...
function createDebugPanel() {
    ...
    debugPanel.innerHTML = `
        <div class="debug-header">
            <h3>🐛 Premium Debug Panel</h3>
            <button onclick="closeDebugPanel()">✕</button>
        </div>
        <div class="debug-content">
            <button onclick="loadDebugInfo()" class="debug-btn">Load Debug Info</button>
            <button onclick="checkPremiumStatus()" class="debug-btn">Check Premium Status</button>
            <button onclick="checkAuthStatus()" class="debug-btn">Check Auth Status</button>
...
async function loadDebugInfo() {
    try {
        ...
        output.innerHTML = `
            <h4>🔍 Premium Debug Information</h4>
            <pre>${JSON.stringify(data, null, 2)}</pre>
        `;
    } catch (error) {
        const output = document.getElementById('debug-output');
        output.innerHTML = `<div class="error">Error loading debug info: ${error.message}</div>`;
    }
}
...
async function checkPremiumStatus() {
    try {
        ...
        const output = document.getElementById('debug-output');
        output.innerHTML = `
            <h4>💎 Premium Status Check</h4>
            <pre>${JSON.stringify(data, null, 2)}</pre>
        `;
    } catch (error) {
        const output = document.getElementById('debug-output');
        output.innerHTML = `<div class="error">Error checking premium status: ${error.message}</div>`;
    }
}
...
async function checkAuthStatus() {
    try {
        ...
        const output = document.getElementById('debug-output');
        output.innerHTML = `
            <h4>🔐 Auth Status Check</h4>
            <pre>${JSON.stringify(data, null, 2)}</pre>
        `;
    } catch (error) {
        const output = document.getElementById('debug-output');
        output.innerHTML = `<div class="error">Error checking auth status: ${error.message}</div>`;
    }
}

// Add debug panel trigger (Ctrl+Shift+D)
...
// Function to show debug info on user status
